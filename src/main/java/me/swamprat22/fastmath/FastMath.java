package me.swamprat22.fastmath;

/**
 * A much faster math library. 99% of this is taken from ApacheMath, However
 * i've improved a few methods myself.
 */
public final class FastMath {
     ...
     */
    private static final double[] SINE_TABLE_A = {+0D, +0.1246747374534607d, +0.24740394949913025d,
     ...
     * Sine table (low bits).
     */
    private static final double[] SINE_TABLE_B = {+0D, -4.068233003401932E-9d, +9.755392680573412E-9d,
     ...
     */
    private static final double[] COSINE_TABLE_A = {+1D, +0.9921976327896118d, +0.9689123630523682d,
     ...
     */
    private static final double[] COSINE_TABLE_B = {+0D, +3.4439717236742845E-8d, +5.865827662008209E-8d,
     ...
     */
    private static final double[] TANGENT_TABLE_A = {+0D, +0.1256551444530487d, +0.25534194707870483d,
            +0.3936265707015991d, +0.5463024377822876d, +0.7214844226837158d, +0.9315965175628662d,
            +1.1974215507507324d, +1.5574076175689697d, +2.092571258544922d, +3.0095696449279785d, +5.041914939880371d,
            +14.101419448852539d, -18.430862426757812d,};
    /**
     * Tangent table, used by atan() (low bits).
     */
    private static final double[] TANGENT_TABLE_B = {+0D, -7.877917738262007E-9d, -2.5857668567479893E-8d,
    ...
    /**
     * Eighths. This is used by sinQ, because its faster to do a table lookup than a
     * multiply in this time-critical routine
     */
    ...
    private static final double[] CBRTTWO = {0.6299605249474366, 0.7937005259840998, 1.0, 1.2599210498948732,
            1.5874010519681994};
    /**
     * 0x40000000 - used to split a double into two parts, both with the low order
     * bits cleared. Equivalent to 2^30.
     */
    private static final long HEX_40000000 = 0x40000000L; // 1073741824L

    /*
     * There are 52 bits in the mantissa of a double. For additional precision, the
     * code splits double numbers into two parts, by clearing the low order 30 bits
     * if possible, and then performs the arithmetic on each half separately.
     */
    /**
     ...
     * @return square root of x. If the argument is NaN or less than zero, the
     * result is NaN.
     */
    ...
    /**
     * Compute the signum of a number. The signum is -1 for negative numbers, +1 for
    ...
    public static float signum(final float a) {
        return (a < 0F) ? -1F : ((a > 0F) ? 1F : a); // return +0.0/-0.0/NaN depending on a
    }
     ...
     * 1024.0) * 1024.0 ); Compute z as the exponential of the remaining bits by a
     * polynomial minus one exp(x) = intVal * fracVal * (1 + z)
     * <p>
     * Accuracy: Calculation is done with 63 bits of precision, so result should be
     * correctly rounded for 99.9% of input values, with less than 1 ULP error
     * otherwise.
     *
     * @param x a double
    ...
    private static double exp(double x, double extra, double[] hiPrec) {
        ...
        /*
         * Lookup exp(floor(x)). intPartA will have the upper 22 bits, intPartB will
         * have the lower 52 bits.
         */
        if (x < 0.0) {

            // We don't check against intVal here as conversion of large negative double
            // values
        ...
        /*
         * Get the fractional part of x, find the greatest multiple of 2^-10 less than x
         * and look up the exp function of it. fracPartA will have the upper 22 bits,
         * fracPartB the lower 52 bits.
         */
        ...
        /*
         * epsilon is the difference in x from the nearest multiple of 2^-10. It has a
         * value in the range 0 <= epsilon < 2^-10. Do the subtraction from x as the
         * last step to avoid possible loss of precision.
         */
        double epsilon = x - (intVal + intFrac / 1024.0);

        /*
         * Compute z = exp(epsilon) - 1.0 via a minimax polynomial. z has full double
         * precision (52 bits). Since z < 2^-10, we will have 62 bits of precision when
         * combined with the constant 1. This will be used in the last addition below to
         * get proper rounding.
         */

        /*
         * Remez generated polynomial. Converges on the interval [0, 2^-10], error is
         * less than 0.5 ULP
         */
         ...
         * tempA is exact since intPartA and intPartB only have 22 bits each. tempB will
        ...
        /*
         * Compute the result. (1+z)(tempA+tempB). Order of operations is important. For
         * accuracy add by increasing size. tempA is exact and much larger than the
         * others. If there are extra bits specified from the pow() function, use them.
         */
    ...
    private static double log(final double x, final double[] hiPrec) {
        ...
        if ((exp == -1 || exp == 0) && x < 1.01 && x > 0.99 && hiPrec == null) {
             ...
             * straight polynomial expansion in higer precision.
             */
        ...
        if (hiPrec != null) {
            ...
            /*
             * High precision not required. Eval Remez polynomial using standard double
             * precision
             */
        ...
        /*
         * Relative sizes: lnzb [0, 2.33E-10] lnm[1] [0, 1.17E-7] ln2B*exp [0, 1.12E-4]
         * lnza [0, 9.7E-4] lnm[0] [0, 0.692] ln2A*exp [0, 709]
         */
     ...
     * <p>
     * Returns {@code NaN} if either argument is negative. If {@code base} is 0 and
     * {@code x} is positive, 0 is returned. If {@code base} is positive and
    ...
    /**
     * Compute sine over the first quadrant (0 < x < pi/2). Use combination of table
    ...
    private static double sinQ(double xa, double xb) {
         ...
         *
         * result = sintA + costA*sinEpsA + sintA*cosEpsB + costA*sinEpsB + sintB +
         * costB*sinEpsA + sintB*cosEpsB + costB*sinEpsB;
         *
         * Ranges of elements
         *
         * xxxtA 0 PI/2 xxxtB -1.5E-9 1.5E-9 sinEpsA -0.0625 0.0625 sinEpsB -6E-11 6E-11
         * cosEpsA 1.0 cosEpsB 0 -0.0625
         *
         */
        ...
        /*
         * t = sintA*cosEpsB; c = a + t; d = -(c - a - t); a = c; b = b + d; t =
         * costA*sinEpsB; c = a + t; d = -(c - a - t); a = c; b = b + d;
         */

        b = b + sintB + costB * sinEpsA + sintB * cosEpsB + costB * sinEpsB;
        /*
         * t = sintB; c = a + t; d = -(c - a - t); a = c; b = b + d; t = costB*sinEpsA;
         * c = a + t; d = -(c - a - t); a = c; b = b + d; t = sintB*cosEpsB; c = a + t;
         * d = -(c - a - t); a = c; b = b + d; t = costB*sinEpsB; c = a + t; d = -(c - a
         * - t); a = c; b = b + d;
         */
    ...
    /**
     * Compute cosine in the first quadrant by subtracting input from PI/2 and then
     * calling sinQ. This is more accurate as the input approaches PI/2.
     *
    ...
    private static double tanQ(double xa, double xb, boolean cotanFlag) {
        ...
        /*
         * double est = (sina+sinb)/(cosa+cosb); double err = (sina - cosa*est) + (sinb
         * - cosb*est); est += err/(cosa+cosb); err = (sina - cosa*est) + (sinb -
         * cosb*est);
         */
    ...
    /**
     * Reduce the input argument using the Payne and Hanek method. This is good for
     * all inputs 0.0 < x < inf Output is remainder after dividing by PI/2 The
     * result array should contain 3 numbers. result[0] is the integer portion, so
     * mod 4 this gives the quadrant. result[1] is the upper bits of the remainder
    ...
    private static void reducePayneHanek(double x, double[] result) {
        ...
        /*
         * prodA, prodB now contain the remainder as a fraction of PI. We want this as a
         * fraction of PI/2, so use the following steps: 1.) multiply by 4. 2.) do a
         * fixed point muliply by PI/4. 3.) Convert to floating point. 4.) Multiply by 2
         */
    ...
    private static double atan(double xa, double xb, boolean leftPlane) {
        ...
        /*
         * yb = -0.09001346640161823; yb = yb * epsA2 + 0.11110718400605211; yb = yb *
         * epsA2 + -0.1428571349122913; yb = yb * epsA2 + 0.19999999999273194; yb = yb *
         * epsA2 + -0.33333333333333093; yb = yb * epsA2 * epsA;
         */
    ...
    public static double cbrt(double x) {
        ...
        // Scale down x for the purpose of doing newtons method. This avoids over/under
        // flows.
        final double xs = x / (p2 * p2 * p2);
        est += (xs - est * est * est) / (3 * est * est);
        est += (xs - est * est * est) / (3 * est * est);

        // Do one round of Newton's method in extended precision to get the last bit
        // right.
    ...
    public static double scalb(final double d, final int n) {
        ...
        // first simple and fast handling when 2^n can be represented using normal
        // numbers
        if ((n > -1023) && (n < 1024)) {
     ...
     * representable number strictly greater than {@code d} is returned; if less,
     * then the largest representable number strictly less than {@code d} is
     * returned.
     * </p>
     * <p>
     ...
     *
     * @param d         base number
     * @param direction (the only important thing is whether {@code direction} is
     ...
     * representable number strictly greater than {@code f} is returned; if less,
     * then the largest representable number strictly less than {@code f} is
     * returned.
     * </p>
     * <p>
    ...
    public static double floor(final double x) {
        ...
        if (Double.isNaN(x) || x >= TWO_POWER_52 || x <= -TWO_POWER_52) {
            return x;
        }
    ...
    /**
     * Computes the remainder as prescribed by the IEEE 754 standard. The remainder
     * value is mathematically equal to {@code x - y*n} where {@code n} is the
     * mathematical integer closest to the exact mathematical value of the quotient
     * {@code x/y}. If two mathematical integers are equally close to {@code x/y}
     * then {@code n} is the integer that is even.
     * <ul>
     * <li>If either operand is NaN, the result is NaN.</li>
     * <li>If the result is not NaN, the sign of the result equals the sign of the
     * dividend.</li>
    ...
    /**
     * Finds q such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt; r
     * &lt;= 0 if b &lt; 0.
     * <p>
     * This methods returns the same value as integer division when a and b are same
     * signs, but returns a different value when they are opposite (i.e. q is
     * negative).
     * </p>
     *
     * @param a dividend
     * @param b divisor
     * @return q such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt;
     * r &lt;= 0 if b &lt; 0
    ...
    /**
     * Finds r such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt; r
     * &lt;= 0 if b &lt; 0.
     * <p>
     * This methods returns the same value as integer modulo when a and b are same
     * signs, but returns a different value when they are opposite (i.e. q is
     * negative).
     * </p>
     *
     * @param a dividend
     * @param b divisor
     * @return r such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt;
     * r &lt;= 0 if b &lt; 0
     ...
     * <p>
     * For float numbers of the form 2<sup>x</sup>, the unbiased exponent is exactly
     * x.
     * </p>
     *
    ...
    /**
     * Enclose large data table in nested static class so it's only loaded on first
     * access.
     */
    ...
    private static class ExpFracTable {
         ...
         * expFracTableA[x] + expFracTableB[x]. 1024 = 2^10
         */
